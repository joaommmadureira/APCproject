...
int main() {
    int opcao_menu; // Armazena a escolhe de operacao do usuario;
    char estado[TAM_ESTADO];
    char cidade[TAM_CIDADE];
    char nome[TAM_MAX];
    int repetidor; // Variavel que vai armazenar a resposta do usuario para repetir a operacao.
    ...
    // Verificacao de continuidade do programa.
    } while (opcao_menu != 9);
...
void cadastrar_estado(char estado[]) {
    ...
    // Declarando o ponteiro do arquivo e abrindo os arquivos chamando a funcao de abrir arquivos.
    FILE *arquivo;
...
void cadastrar_cidade(char cidade[], char estado[]) {
    ...
    // Retornando o resultado da operacao.
    if(ferror(arquivo)) 
    {        
        fclose(arquivo);
...
/* Funcao que verifica se o usuario cadastrado ja existe. Compara todas as informacoes e caso tiverem identicas reconhece como ja cadastrado. 
...
// Funcao para validar o nome selecionado, recebe o nome digitado como paramentro. Retorna 0 quando invalido e 1 quando valido.
int validar_nome(char nome[]) {
    int i;

    // Verificando se o nome contem algo alem de letras e espacos.
...
// Funcao para validar o sexo selecionado, recebe o sexo como paramentro. Retorna 0 quando invalido e 1 quando valido.
int validar_sexo(char sexo[]) {
...
// Funcao para validar a data escrita, recebe uma struct Data como parâmetro .Retorna 0 quando invalida e 1 quando valida.
...
/* Funcao que verifica se o estado passado e uma sigla e se existe no arquivo de estados. Retorna 1 se encontrar e 0 se nao 
...
void listar_pessoas_cidade(char cidade[]) {
    ...
    if(i >= 1) 
    {
        ...
        // Verificando qual cidade o usuario deseja listar.
        printf("\nSelecione o numero da cidade que deseja consultar: ");
...
void consultar_pessoas_nome(char nome[]) {
    ...
    if(i >= 1)
    {
        ...
        printf("\nSelecione o registro que deseja consultar: ");
...
/* Funcao para deletar um cadastro do sistema. Recebe o nome (completo ou parcialmente) da pessoa que deseja deletar.
...
void deletar_pessoas(char nome[]) {
    ...
    printf("\nSelecione qual cadastro deseja excluir: ");
    int pessoa_escolhida;
    scanf("%d", &pessoa_escolhida);

    // Confirmando a decisao do usurario.
    printf("\nTem certeza que deseja excluir o registro de -> %s <- ? Isso apagara todos os dados desse cadastro do sistema.\nEssa operacao nao podera ser desfeita!\n", deixar_maiusculo(pessoas[pessoa_escolhida - 1]));
    printf("\t1 - SIM\n\t2 - NAO\n- ");
    int escolha_final;
    ...
    if(escolha_final == 1)
    {
        ...
        while(fread(&pessoa, sizeof(Cadastro), 1, arquivo)) {
            ...
            if (strcmp(pessoas[pessoa_escolhida - 1], pessoa.nome) == 0
                && strcmp(cidades[pessoa_escolhida - 1], pessoa.cidade) == 0
                && strcmp(estados[pessoa_escolhida - 1], pessoa.estado) == 0
...
// Funcao para calcular a idade de uma pessoa com base na struct Data passada como parãmetro. Retorna a idade calculada.
int calcular_idade(Data nascimento) {
    ...
    int idade = 0; // Variável que irá armazenar a idade e ser retornada no final da funcao.
...
/* Funcao para comparar duas strings. Recebe a string que se deseja consultar e a string de base. Retorna 0 se caso sejam identicas, 
...
// Funcao para deixar a primeira letra de cada palavra maiuscula e o resto minusculo.
char* deixar_maiusculo(char texto[]) {
    int i;
